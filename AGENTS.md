...
- Remove unecssary instructions, like validations or checks, let it fail, thats ok because our priority is reducing bytecode score.
...
- Inline expressions and unpacking beats multiple temporary variables in tight loops; each additional instruction can raise the count.
- Remove unused imports or helper functions; every definition creates more bytecode even if never executed.
- Comments should stay succinct; AGENTS.md already expects block separators, so only add them when they explain behavior necessary for understanding the lean code.
- Dont access environment variables directly, either pass them as cmd line args or use django settings if inside django app.
...
- Add more comments, especially block level comments that visually show what a particular block does.
- Comments and blank lines do not add bytecode; feel free to use them so long as they stay focused and concise.
- Use meaningful naming convention in code.
- Add new lines where necessary to improve code readability.

## Logging
- Implement logging from the outset. Provide real-time visibility into what’s happening, keeping output concise and information-dense without overwhelming the terminal.
...
- Don't do rebase, I want to preserve exact history. This is again from the mindset to prefer explicitness even if its ugly.

## SSH usage in python scripts
